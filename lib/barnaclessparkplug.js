...
const EVENT_FILTER_CLASSES = { dynamb: DynambFilter };
...
class BarnaclesSparkplug {
  ...
  handleEvent(name, data) {
    ...
    else if(name === 'raddec') { // TODO: move to function
      if(Array.isArray(data.position) && (data.position.length >= 2)) {
        let dynamb = { deviceId: data.transmitterId,
                       deviceIdType: data.transmitterIdType,
                       position: data.position,
                       timestamp: data.timestamp };
        return handleDynamb(self, dynamb);
      }
    }
    else if(name === 'spatem') {
      return; // TODO: position as dynamb
    }
  }
}
